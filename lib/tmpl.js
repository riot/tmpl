...
var tmpl = (function () {
  ...
  var cache = { '@#1@': function () { return 'name' } }, // cache of template getters
   ...
   *  You don't need escape inner brackets in expressions, except very specific
   *  cases.
   *
   * NOTE:
   *  There are only two contexts accesible to the returned function: `this`,
   *  received by tmpl and assigned to the 'data' parameter, and the global
   *  object (`global` in node.js or `window` in browsers).
   *
   * @param {string} str - One expression, or template with zero or more expressions
   ...
   *
   * @param {string} str - The template string, can include zero or more expressions.
   * @returns {string} - Code for the getter body.
   * @private
   */
   ...
   * @returns {Array} - Array of `[ttext], [expression]` sequences.
   * @private
   */
  function _splitByPairs(str) {
    ...
    */
    ...
    var
      ...
      eb  = brackets(6),          // regexp, matches current escaped riot brackets
      re  = brackets(7),          // regexp, matches opening bracket
                                  // regexp for riot brackets
      REs = [re, newRegExp(brackets(8) + '|' + RE_QBLOCKS.source, REGLOB)]
                                  // REs will find an opening bracket before a qblock
    start = isexpr = 0
     ...
     *
     * @param {string} str - The whole string for search within.
     * @param {string} opench - The openning bracket.
     * @param {string} chpos - Position of the openning bracket.
     * @returns {number} - Position following the closing bracket or the end of
   ...
   * @param {number} mode - 0:return raw value, 1:returns text
   * @param {Array} qstr - Saved quoted strings
   * @returns {string} - Code for get the expression value
   * @private
   */
   ...
   *
   * @param {string} str - Teamplate expression
   * @param {Array} list - Array for extracted shorthands
   * @returns {number} - Count of shorthands, zero if str is not a shorthand list
   * @private
   */
  ...
  var
    ...
    JS_VARSTART = newRegExp(
        ...
        '(' + SRE_VARNAME + ')'           // $2: var name or falsy literal value
      ),
   ...
   *
   * @param {string} expr - The expression
   * @param {bool} asText - If true, convert falsy values to '', except zero.
