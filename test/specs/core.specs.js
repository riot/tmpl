...
describe('riot-tmpl', function () {
  ...
  describe('anonymous and arrow callbacks', function () {
    ...
    // it('evaluate simple method with same-line arrow callback, unbracketed return, with global var', function () {
    //   const evaluated = render('{{ $json["field"].join("-") }}')
    //   expect(evaluated).eql('1-2-3')
    // })

    // it('evaluate simple method with same-line arrow callback, unbracketed return, with data var', function () {
    //   data.$jsondata = { field: [1, 2, 3] }
    //   const evaluated = render('{{ $jsondata["field"].join("-") }}')
    //   expect(evaluated).eql('1-2-3')
    // })

    it('evaluate callback method with same-line arrow callback, unbracketed return, with global var', function () {
      const evaluated = render('{{ $json["field"].filter((v) => v > 2) }}')
      expect(evaluated).eql([3])
    })

    it('evaluate callback method with same-line arrow callback, unbracketed return, with data var', function () {
      data.$jsondata = { field: [1, 2, 3] }
      const evaluated = render('{{ $jsondata["field"].filter((v) => v > 2) }}')
      expect(evaluated).eql([3])
    })
