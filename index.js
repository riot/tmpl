...
var brackets = (function (defaults) {
  ...
  return function _brackets(reOrIdx) {
    ...
    var s = typeof riot == 'object' && riot.settings.brackets ? riot.settings.brackets : defaults
...
var tmpl = (function () {
  ...
  function splitByPairs(str) {
    ...
    var parts = [],
      ...
      REs = [re, newRegExp(brackets(6) + '|' + RE_QBLOCKS.source, 'g')]
  ...
  function skipBracketedPart(str, opench, chpos, qskip) {
    ...
    if (qskip)
      recch = newRegExp(recch.source + '|' + RE_QBLOCKS.source, 'g')
    else chpos++
    ...
    while (level && (match = recch.exec(str))) {
      ...
      if (match[1]) match[1] !== opench ? --level : qskip ? qskip = 0 : ++level
    }
