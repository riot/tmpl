...
var brackets = (function (UNDEF) {
  ...
  var
    ...
    /**
     * Matches valid multiline comments in almost all their forms.
     * @const {RegExp}
     * @static
     */
    R_MLCOMMS = /\/\*[^*]*\*+(?:[^*\/][^*]*\*+)*\//g,
    ...
    /**
     * Matches quoted strings. Don't care about inner EOLs.
     * @const {RegExp}
     * @static
     */
    R_STRINGS = /"[^"\\]*(?:\\[\S\s][^"\\]*)*"|'[^'\\]*(?:\\[\S\s][^'\\]*)*'/g,
  ...
  var
    ...
    _settings                     // mirros `riot.settings`
   ...
   * @param   {Array} [bp] - Custom escaped brackets in elements 2-3
   ...
   * For the default brackets, returns a reference to an inner static array.
   * @param   {string} pair - String with the brackets pair (cannot be falsy)
