...
//#if PURE_PCEXPR
  ...
  //
  //  0,1 - current left (0) or right (1) brackets characters.<br>
  //  2,3 - current left (2) or right (3) _escaped_ brackets characters.<br>
  //    4 - regex matching an precompiled expression
  ...
  function _set(pair) {}   // dummy function, brackets can be changed with pce

  // With a regex, returns the regex with the default riot brackets replaced with
  // the internal ones.
  function _brackets(reOrIdx) {
    return reOrIdx instanceof RegExp ?
      new RegExp(
        reOrIdx.source.replace(/{/g, '{#').replace(/}/g, '#}'),
        reOrIdx.global ? 'g' : ''
      ) :
      _bp[reOrIdx]
  }
...
//#else
  ...
  var
    ...
    _bp = [                 // eslint-disable-line no-redeclare
      ...
      /\\({|})/g,           // 5: for _split(), matches one escaped bracket
      /(\\?)({)/g           // 6: for _split(), matches openning bracket
    ],
    REGLOB = 'g'
  ...
  function reWrite(r, p) {
    //#if DEBUG
    var s = r.source.replace(/{/g, p[2]).replace(/}/g, p[3])
    return new RegExp(s, r.global ? REGLOB : '')
    //#else
    return new RegExp(      // eslint-disable-line no-unreachable
  ...
  function _split(str, bp) {
    ...
    var
      ...
      re = bp[6]                  // start with opening bracket
    ...
    while (match = re.exec(str)) {
      ...
      if (!match[1]) {                      // ignore it if have an escape char
        ...
        re = bp[6 + (isexpr ^= 1)]          // switch mode and swap regexp
  ...
  // Creates an array with strings and premake regexes or,
  ...
  function _set(pair) {               // eslint-disable-line no-redeclare
  ...
  function _brackets(reOrIdx) {       // eslint-disable-line no-redeclare
...
//#endif // PURE_PCEXPR
  ...
  // mirror the riot.settings, you can assign this if riot is not in context
